...
export default function Dashboard() {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center"><div className="animate-spin h-8 w-8 border-4 border-blue-500 border-t-transparent rounded-full"></div></div>;
  ...
  const themeClass = isMedico ? "theme-dash-medico" : "theme-dash-paciente";
  return (
    <div className={`min-h-screen flex bg-[#f1f5f9] ${themeClass}`}>
      {sidebarOpen && <div className="fixed inset-0 bg-black/50 z-40 lg:hidden" onClick={() => setSidebarOpen(false)} />}

      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 w-72 lg:w-64 bg-white border-r border-[var(--dash-border)] z-50 transform transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 lg:static lg:shrink-0`}>
          ...
          <nav className="flex-1 space-y-1">
            {navItems.map((item) => (
              <button key={item.id} onClick={() => handleTabChange(item.id)}
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-semibold transition-colors text-sm ${tab === item.id ? "bg-[var(--dash-accent)]/10 text-[var(--dash-text)]" : "text-slate-500 hover:bg-[var(--dash-accent-light)]"}`}>
                <span className="material-icons-outlined text-xl">{item.icon}</span>{item.label}
              </button>
            ))}
            <div className="pt-4 mt-4 border-t border-[var(--dash-border)] space-y-1">
              <Link href="/agentes" className="flex items-center gap-3 px-4 py-3 text-slate-500 hover:bg-[var(--dash-accent-light)] rounded-xl transition-colors text-sm">
                <span className="material-icons-outlined text-xl">smart_toy</span>Agentes IA
              </Link>
              <a className="flex items-center gap-3 px-4 py-3 text-slate-500 hover:bg-[var(--dash-accent-light)] rounded-xl transition-colors text-sm" href="https://wa.me/12763294935">
                <span className="material-icons-outlined text-xl">chat</span>Hablar con Aura
              </a>
              <Link href="/" className="flex items-center gap-3 px-4 py-3 text-slate-500 hover:bg-[var(--dash-accent-light)] rounded-xl transition-colors text-sm">
                <span className="material-icons-outlined text-xl">home</span>Inicio
              </Link>
            </div>
          </nav>
          <div className="mt-auto p-3 bg-[var(--dash-accent-light)] rounded-xl">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-[var(--dash-accent)]/20 rounded-full flex items-center justify-center text-[var(--dash-accent)] font-bold text-sm shrink-0">
                {(user?.nombre?.[0] || "")}{(user?.apellido?.[0] || "")}
              </div>
              <div className="min-w-0">
                <p className="text-sm font-bold text-[var(--dash-text-dark)] truncate">{user?.nombre} {user?.apellido}</p>
                <p className="text-xs text-[var(--dash-text)] capitalize">{user?.rol}</p>
              </div>
            </div>
            ...
            <p className="text-center text-[10px] text-slate-400 mt-3">Powered by <span className="font-semibold text-[var(--dash-accent)]">AINovaX</span></p>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 min-w-0">
        <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-lg border-b border-[var(--dash-border)] px-4 py-3 lg:px-8 lg:py-4">
            ...
            <div className="space-y-6">
              ...
              {(() => {
                ...
                if (upcoming.length > 0) {
                  const c = upcoming[0];
                  return (
                    <div className="bg-[var(--dash-accent-dark)] text-white rounded-xl p-5 lg:p-8">
                      <span className="inline-block px-2 py-0.5 bg-[var(--dash-accent)]/20 text-[var(--dash-accent-muted)] text-[10px] font-bold rounded-full mb-2 uppercase">Próxima Cita</span>
                      <h2 className="text-lg lg:text-2xl font-bold">{isMedico ? c.pacienteNombre : c.medicoNombre}</h2>
                      <p className="text-[var(--dash-accent-muted)]/90 text-sm">{c.especialidad} • {c.tipo}</p>
                      <div className="flex flex-wrap gap-3 mt-2 text-sm">
                        ...
                        <span className="flex items-center gap-1 text-[var(--dash-accent-muted)]"><span className="material-icons-outlined text-sm">schedule</span>{c.hora}</span>
                        <span className={`px-2 py-0.5 text-[10px] font-bold rounded uppercase ${c.estado === "confirmada" ? "bg-[var(--dash-accent)]/20 text-[var(--dash-accent-muted)]" : "bg-amber-500/20 text-amber-300"}`}>{c.estado}</span>
                      </div>
                    </div>
                  );
                }
                return (
                  <div className="bg-white p-6 rounded-xl border border-[var(--dash-border)] text-center">
                    ...
                    {!isMedico && <button onClick={() => { setTab("citas"); setShowAgendarCita(true); }} className="bg-[var(--dash-accent)] text-white font-bold py-2 px-6 rounded-xl text-sm">Agendar Cita</button>}
                  </div>
                );
              })()}
              ...
              {progresoWA.length > 0 && (
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                    {progresoWA.map((p) => (
                      <div key={p.id} className="bg-white p-5 rounded-xl border border-[var(--dash-border)] relative overflow-hidden">
                        ...
                        <div className="flex items-end justify-between mb-2">
                          <div>
                            ...
                            {p.nota && <p className="text-xs text-[var(--dash-text)] font-bold">Nota: {p.nota}/100</p>}
                          </div>
                          <p className="text-2xl font-bold text-[var(--dash-text)]">{p.porcentaje}%</p>
                        </div>
                        <div className="w-full bg-[var(--dash-accent-light)] rounded-full h-2.5">
              ...
              {isMedico && (
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <button onClick={() => { setTab("formulas"); setShowCrearFormula(true); }} className="bg-white p-4 rounded-xl border border-[var(--dash-border)] hover:border-[var(--dash-accent)]/50 transition text-left">
                    <span className="material-icons-outlined text-[var(--dash-accent)] text-2xl mb-2">medication</span>
                    <p className="font-bold text-sm">Nueva Fórmula</p>
                    <p className="text-xs text-slate-500">Prescribir a un paciente</p>
                  </button>
                  <button onClick={() => { setTab("historia"); setShowCrearHistoria(true); }} className="bg-white p-4 rounded-xl border border-[var(--dash-border)] hover:border-[var(--dash-accent)]/50 transition text-left">
                    <span className="material-icons-outlined text-[var(--dash-accent)] text-2xl mb-2">history_edu</span>
                    <p className="font-bold text-sm">Registrar HC</p>
                    <p className="text-xs text-slate-500">Actualizar historia clínica</p>
                  </button>
                  <button onClick={() => setTab("pacientes")} className="bg-white p-4 rounded-xl border border-[var(--dash-border)] hover:border-[var(--dash-accent)]/50 transition text-left">
                    <span className="material-icons-outlined text-[var(--dash-accent)] text-2xl mb-2">group</span>
          ...
          {tab === "citas" && (
            <div>
              <div className="flex items-center justify-between mb-6">
                ...
                {!isMedico && (
                  <button onClick={() => setShowAgendarCita(true)} className="bg-[var(--dash-accent)] text-white font-bold py-2 px-4 rounded-xl text-sm flex items-center gap-2">
              ...
              {showAgendarCita && (
                <Modal onClose={() => setShowAgendarCita(false)}>
                  ...
                  <div className="flex gap-3 mt-6">
                    ...
                    <button onClick={agendarCita} disabled={saving || !citaForm.fecha || !citaForm.hora || !citaForm.motivo || !citaForm.especialidad} className="flex-1 py-2.5 bg-[var(--dash-accent)] text-white rounded-xl text-sm font-bold disabled:opacity-50">
                      ...
                      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
                        ...
                        <div className="flex flex-wrap items-center gap-2 self-start">
                          <span className={`px-3 py-1 text-xs font-bold rounded-full ${
                            c.estado === "confirmada" ? "bg-[var(--dash-accent-light)] text-[var(--dash-text)]" :
              ...
              {pacientes.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                  {pacientes.map(p => {
                    ...
                    return (
                      <div key={p.id} className="bg-white p-5 rounded-xl border border-[var(--dash-border)]">
                        <div className="flex items-center gap-3 mb-3">
                          <div className="w-12 h-12 bg-[var(--dash-accent)]/20 rounded-full flex items-center justify-center text-[var(--dash-accent)] font-bold">
                        ...
                        <div className="flex gap-2">
                          <button onClick={() => { setFormulaForm({...formulaForm, pacienteId: p.id}); setShowCrearFormula(true); setTab("formulas"); }} className="flex-1 py-2 bg-[var(--dash-accent)]/10 text-[var(--dash-text)] rounded-lg text-xs font-bold hover:bg-[var(--dash-accent)]/20">
                            Prescribir
                          </button>
          ...
          {tab === "formulas" && (
            <div>
              <div className="flex items-center justify-between mb-6">
                ...
                {isMedico && (
                  <button onClick={() => setShowCrearFormula(true)} className="bg-[var(--dash-accent)] text-white font-bold py-2 px-4 rounded-xl text-sm flex items-center gap-2">
              ...
              {showCrearFormula && isMedico && (
                <Modal onClose={() => setShowCrearFormula(false)}>
                  ...
                  <div className="space-y-4 max-h-[60vh] overflow-y-auto">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <button onClick={addMedicamento} className="text-xs text-[var(--dash-accent)] font-bold flex items-center gap-1">
                  ...
                  <div className="flex gap-3 mt-6">
                    ...
                    <button onClick={crearFormula} disabled={saving || !formulaForm.pacienteId || !formulaForm.diagnostico || !formulaForm.medicamentos[0].nombre} className="flex-1 py-2.5 bg-[var(--dash-accent)] text-white rounded-xl text-sm font-bold disabled:opacity-50">
              ...
              {formulas.length === 0 ? (
                ...
                <div className="space-y-4">
                  {formulas.map((f) => (
                    <div key={f.id} className="bg-white p-4 lg:p-6 rounded-xl border border-[var(--dash-border)]">
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <span className={`px-2 py-0.5 text-[10px] font-bold rounded uppercase ${f.estado === "activa" ? "bg-[var(--dash-accent-light)] text-[var(--dash-text)]" : "bg-amber-100 text-amber-700"}`}>{f.estado}</span>
                      </div>
          ...
          {tab === "historia" && (
            <div>
              <div className="flex items-center justify-between mb-6">
                ...
                {isMedico && (
                  <button onClick={() => setShowCrearHistoria(true)} className="bg-[var(--dash-accent)] text-white font-bold py-2 px-4 rounded-xl text-sm flex items-center gap-2">
              ...
              {showCrearHistoria && isMedico && (
                <Modal onClose={() => setShowCrearHistoria(false)}>
                  ...
                  <div className="flex gap-3 mt-6">
                    ...
                    <button onClick={crearHistoria} disabled={saving || !historiaForm.pacienteId || !historiaForm.diagnostico} className="flex-1 py-2.5 bg-[var(--dash-accent)] text-white rounded-xl text-sm font-bold disabled:opacity-50">
              ...
              {historia.length === 0 ? (
                ...
                <div className="space-y-3">
                  {historia.map((h) => (
                    <div key={h.id} className="bg-white p-4 lg:p-6 rounded-xl border border-[var(--dash-border)]">
                      ...
                      {h.signos && Object.values(h.signos).some(v => v) && (
                        <div className="flex flex-wrap gap-2 mt-3">
                          {Object.entries(h.signos).map(([k, v]) => v && (
                            <span key={k} className="px-2 py-1 bg-[var(--dash-accent-light)] rounded text-xs font-medium capitalize">{k}: {v}</span>
                          ))}
                        </div>
                      )}
                    </div>
              ...
              {cursoActivo && cursoDetalle ? (() => {
                ...
                return (
                  <div>
                    ...
                    <div className="bg-white p-5 lg:p-6 rounded-xl border border-[var(--dash-border)] mb-6">
                      ...
                      <div className="flex flex-wrap items-center gap-4 mt-2 mb-4">
                        <div className="flex items-center gap-2 text-sm">
                          <span className="material-icons-outlined text-[var(--dash-accent)] text-lg">check_circle</span>
                          <span className="font-bold">{completedCount}/{totalItems}</span> <span className="text-slate-500">ítems completados</span>
                        </div>
                        <div className="flex items-center gap-2 text-sm">
                          <span className="font-bold text-[var(--dash-text)]">{progreso}%</span>
                        </div>
                        {ins.estado === "completado" && (
                          <span className="px-3 py-1 bg-[var(--dash-accent-light)] text-[var(--dash-text)] text-xs font-bold rounded-full">✅ CERTIFICADO</span>
                        )}
                      </div>
                      <div className="w-full bg-[var(--dash-accent-light)] rounded-full h-3">
                        <div className="h-3 rounded-full bg-[var(--dash-accent)] transition-all" style={{ width: `${progreso}%` }}></div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4 mb-8">
                      {cursoDetalle.modulos.map((mod, mi) => {
                        ...
                        return (
                          <div key={mi} className="bg-white rounded-xl border border-[var(--dash-border)] overflow-hidden">
                            <div className={`px-4 lg:px-5 py-3 flex items-center justify-between ${modComplete ? "bg-[var(--dash-accent-light)]" : "bg-slate-50"}`}>
                              <h3 className="font-bold text-sm flex items-center gap-2">
                                {modComplete && <span className="material-icons-outlined text-[var(--dash-accent)] text-lg">check_circle</span>}
                                {mod.titulo}
                              </h3>
                              ...
                              {mod.items.map((item, ii) => {
                                ...
                                return (
                                  <div key={ii} className="w-full flex items-center gap-3 px-4 lg:px-5 py-3 text-left">
                                    <div className={`w-5 h-5 rounded border-2 flex items-center justify-center shrink-0 ${done ? "bg-[var(--dash-accent)] border-[var(--dash-accent)]" : "border-slate-200 bg-slate-50"}`}>
                    ...
                    {/* Evaluación */}
                    <div className="bg-white rounded-xl border border-[var(--dash-border)] p-5 lg:p-6">
                      <h3 className="text-lg font-bold mb-2 flex items-center gap-2">
                      ...
                      {ins.estado === "completado" ? (
                        <div className="bg-[var(--dash-accent-light)] p-4 rounded-xl text-center">
                          <span className="material-icons-outlined text-[var(--dash-accent)] text-5xl mb-2">workspace_premium</span>
                          <p className="font-bold text-[var(--dash-text-dark)] text-lg">¡Curso Aprobado!</p>
                          <p className="text-sm text-[var(--dash-text)]">Nota: {ins.evaluacionNota}% — Certificado emitido</p>
                        </div>
                      ) : !allItemsDone ? (
                        ...
                        <div>
                          ...
                          <button onClick={() => { setShowEvaluacion(true); setRespuestas({}); setEvalResult(null); }} className="bg-[var(--dash-accent)] text-white font-bold py-3 px-6 rounded-xl text-sm flex items-center gap-2">
                            <span className="material-icons-outlined">assignment</span>
                            {ins.evaluacionNota !== null ? "Reintentar Evaluación" : "Presentar Evaluación"}
                          </button>
                        </div>
                      ) : evalResult ? (
                        <div className={`p-4 rounded-xl text-center ${evalResult.aprobado ? "bg-[var(--dash-accent-light)]" : "bg-red-50"}`}>
                          <span className={`material-icons-outlined text-5xl mb-2 ${evalResult.aprobado ? "text-[var(--dash-accent)]" : "text-red-400"}`}>
                            {evalResult.aprobado ? "workspace_premium" : "sentiment_dissatisfied"}
                          </span>
                          <p className={`font-bold text-lg ${evalResult.aprobado ? "text-[var(--dash-text-dark)]" : "text-red-700"}`}>
                            {evalResult.aprobado ? "¡Aprobaste!" : "No alcanzaste el puntaje"}
                          </p>
                          <p className={`text-sm ${evalResult.aprobado ? "text-[var(--dash-text)]" : "text-red-600"}`}>
                        ...
                        <div>
                                  ...
                                  {q.opciones.map((op, oi) => (
                                    <button key={oi} onClick={() => setRespuestas({ ...respuestas, [qi]: oi })}
                                      className={`w-full text-left px-4 py-2.5 rounded-lg text-sm transition border ${
                                        respuestas[qi] === oi
                                          ? "bg-[var(--dash-accent)]/10 border-[var(--dash-accent)] font-bold"
                          ...
                          <div className="flex gap-3">
                            ...
                            <button
                              ...
                              className="flex-1 py-2.5 bg-[var(--dash-accent)] text-white rounded-xl text-sm font-bold disabled:opacity-50">
                ...
                <div>
                    ...
                    <div className="mb-8">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                        {inscripciones.map((ins) => {
                          ...
                          return (
                            <div key={ins.id} className="bg-white p-5 rounded-xl border border-[var(--dash-border)] hover:border-[var(--dash-accent)]/30 transition cursor-pointer" onClick={() => abrirCurso(ins.cursoId)}>
                              <div className="flex items-start justify-between mb-2">
                                <div className="text-3xl">{curso?.imagen || "📚"}</div>
                                {ins.estado === "completado" && <span className="px-2 py-0.5 bg-[var(--dash-accent-light)] text-[var(--dash-text)] text-[10px] font-bold rounded-full">CERTIFICADO</span>}
                              </div>
                              ...
                              <div className="flex justify-between text-xs mb-2">
                                ...
                                <span className="font-bold text-[var(--dash-text)]">{pct}%</span>
                              </div>
                              <div className="w-full bg-[var(--dash-accent-light)] rounded-full h-2"><div className="h-2 rounded-full bg-[var(--dash-accent)]" style={{ width: `${pct}%` }}></div></div>
                              <p className="text-xs text-[var(--dash-accent)] font-bold mt-3 flex items-center gap-1">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                        {progresoWA.map((p) => (
                          <div key={p.id} className="bg-gradient-to-br from-[#25D366]/5 to-white p-5 rounded-xl border border-[#25D366]/20">
                            ...
                            {p.nota && <p className="text-xs text-[var(--dash-text)] font-bold">Última nota: {p.nota}/100</p>}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                    {cursos.map((curso) => {
                      ...
                      return (
                        <div key={curso.id} className="bg-white p-5 rounded-xl border border-[var(--dash-border)] hover:border-[var(--dash-accent)]/50 transition-all">
                          ...
                          {enrolled ? (
                            <button onClick={() => abrirCurso(curso.id)} className="w-full py-2 rounded-lg text-xs font-bold bg-[var(--dash-accent-light)] text-[var(--dash-text)]">
                              📖 Ir al curso
                            </button>
                          ) : (
...
function Stat({ label, value }: { label: string; value: number }) {
  return (
    <div className="bg-white p-4 rounded-xl border border-[var(--dash-border)]">
...
function Empty({ icon, title, subtitle }: { icon: string; title: string; subtitle: string }) {
  return (
    <div className="bg-white p-8 rounded-xl border border-[var(--dash-border)] text-center">
