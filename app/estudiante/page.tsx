...
export default function Estudiante() {
  ...
  return (
    <div className="flex min-h-screen bg-[#f6f8f6]" style={{ fontFamily: "'Manrope', sans-serif" }}>
      ...
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 w-72 lg:w-64 bg-white border-r border-[#13ec5b]/10 z-50 transform transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 lg:static lg:shrink-0 flex flex-col`}>
        <div className="p-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-[#13ec5b]/20 rounded-lg flex items-center justify-center">
              <span className="material-icons-outlined text-[#13ec5b] font-bold">school</span>
            </div>
            <div>
          ...
          {navItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors text-sm ${
                tab === item.id ? "bg-[#13ec5b]/15 border-r-4 border-[#13ec5b] font-semibold" : "text-[#4c5a52] hover:bg-[#13ec5b]/5"
              }`}
            >
        ...
        <div className="p-4 mt-auto">
          <div className="bg-[#13ec5b]/10 rounded-xl p-3 border border-[#13ec5b]/20">
            <p className="text-xs font-semibold mb-1">Impacto Social</p>
            <div className="flex items-center gap-2">
              <span className="material-icons-outlined text-[#13ec5b] text-sm">volunteer_activism</span>
              <span className="text-sm font-bold">1,250 pts</span>
            </div>
          </div>
          <div className="mt-3 flex items-center gap-3 px-2">
            <div className="w-9 h-9 bg-[#13ec5b]/20 rounded-full flex items-center justify-center text-[#13ec5b] font-bold text-xs shrink-0">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Mobile header */}
        <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-lg border-b border-[#13ec5b]/10 px-4 py-3 lg:hidden">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <button onClick={() => setSidebarOpen(true)} className="p-2 -ml-2 text-slate-600 hover:bg-emerald-50 rounded-lg">
                <span className="material-icons-outlined">menu</span>
              </button>
              <span className="text-lg font-bold">CVFDI</span>
            </div>
            <div className="w-8 h-8 bg-[#13ec5b]/20 rounded-full flex items-center justify-center text-[#13ec5b] font-bold text-xs">
              {(user?.nombre?.[0] || "A")}{(user?.apellido?.[0] || "R")}
            </div>
          </div>
        </header>
        ...
        <div className="flex flex-1 overflow-hidden">
          <main className="flex-1 overflow-y-auto p-4 lg:p-8">
            ...
            <section className="mb-8 lg:mb-10">
              <div className="flex items-center justify-between mb-4 lg:mb-6">
                ...
                <button className="text-[#13ec5b] text-xs lg:text-sm font-bold flex items-center gap-1">Ver detalles <span className="material-icons-outlined text-sm">arrow_forward</span></button>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {competencias.map((c, i) => (
                  <div key={i} className="bg-white p-4 lg:p-6 rounded-xl border border-[#13ec5b]/10 flex items-center gap-4 shadow-sm">
                    <div className="relative w-14 h-14 lg:w-16 lg:h-16 shrink-0">
                      <svg className="w-full h-full" style={{ transform: "rotate(-90deg)" }}>
                        <circle cx="50%" cy="50%" r="44%" fill="transparent" stroke="#13ec5b" strokeWidth="6" opacity="0.1" />
                        <circle cx="50%" cy="50%" r="44%" fill="transparent" stroke="#13ec5b" strokeWidth="6" strokeDasharray="175" strokeDashoffset={c.offset} strokeLinecap="round" />
                      </svg>
            ...
            <section>
              ...
              <div className="space-y-4">
                {cursos.map((curso, i) => (
                  <div key={i} className="bg-white p-4 lg:p-5 rounded-xl border border-[#13ec5b]/10 shadow-sm group hover:border-[#13ec5b]/30 transition-all">
                    <div className="flex gap-4 items-start">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          <span className="px-2 py-0.5 bg-[#13ec5b]/10 text-[#13ec5b] text-[10px] font-bold rounded uppercase">{curso.nivel}</span>
                        ...
                        <div className="mt-2 flex items-center gap-3">
                          <div className="flex-1 bg-slate-100 h-2 rounded-full overflow-hidden">
                            <div className="bg-[#13ec5b] h-full rounded-full" style={{ width: `${curso.progreso}%` }}></div>
                          </div>
                    ...
                    <div className="mt-3 pt-3 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                      ...
                      <button className={`font-bold py-2 px-5 rounded-lg text-xs lg:text-sm transition-all shrink-0 ${
                        i === 0 ? "bg-[#13ec5b] hover:bg-[#13ec5b]/90 text-white shadow-lg shadow-[#13ec5b]/20" : "bg-white border border-[#13ec5b] text-[#13ec5b]"
                      }`}>
            ...
            <div className="xl:hidden mt-8 space-y-6">
              <div>
                <h3 className="text-lg font-bold flex items-center gap-2 mb-4">
                  <span className="material-icons-outlined text-[#13ec5b]">calendar_today</span>
                  Próximas Tutorías
                </h3>
                  ...
                  {[
                    ...
                    <div key={i} className="p-4 rounded-xl bg-white border border-slate-100">
                      <div className="flex justify-between items-start mb-2">
                        <span className={`text-[10px] font-bold px-2 py-0.5 bg-slate-50 rounded-full ${i === 0 ? "text-[#13ec5b]" : "text-slate-400"}`}>{t.when}</span>
              ...
              <div className="bg-[#13ec5b]/5 rounded-2xl p-5 border border-[#13ec5b]/10 text-center">
                ...
                <button className="w-full bg-[#0d1b13] text-white py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2">
                  <span className="material-icons-outlined text-[#13ec5b] text-sm">chat</span>
                  Chat con Mentor
                </button>
              </div>
            </div>
          ...
          <aside className="w-80 bg-white border-l border-[#13ec5b]/10 p-6 overflow-y-auto hidden xl:block shrink-0">
            <div className="mb-8">
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={i} className="p-4 rounded-xl bg-[#f6f8f7] border border-slate-100 hover:border-[#13ec5b]/20 transition-all">
                    <div className="flex justify-between items-start mb-2">
                      <span className={`text-[10px] font-bold px-2 py-0.5 bg-white rounded-full shadow-sm ${i === 0 ? "text-[#13ec5b]" : "text-slate-400"}`}>{t.when}</span>
                      <span className="text-xs text-slate-400">{t.hora}</span>
                    </div>
                    <p className="text-sm font-bold mb-1">{t.titulo}</p>
                    <p className="text-xs text-[#4c9a6c]">Mentor: {t.mentor}</p>
              ...
              <button className="w-full mt-4 text-xs font-bold text-slate-400 py-2 border border-dashed border-slate-200 rounded-lg hover:border-[#13ec5b]/40 hover:text-[#13ec5b] transition-all">+ Agendar nueva sesión</button>
            </div>

            <div className="bg-[#13ec5b]/5 rounded-2xl p-6 border border-[#13ec5b]/10 relative overflow-hidden">
              <div className="absolute -right-4 -top-4 w-20 h-20 bg-[#13ec5b]/10 rounded-full blur-xl"></div>
              <div className="relative z-10 text-center">
                <div className="w-20 h-20 mx-auto rounded-full p-1 border-2 border-[#13ec5b] mb-4">
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={i} className="flex gap-3">
                    <div className={`w-2 h-2 bg-[#13ec5b] rounded-full mt-1.5 shrink-0 ${i > 0 ? "opacity-40" : ""}`}></div>
                    <div>
